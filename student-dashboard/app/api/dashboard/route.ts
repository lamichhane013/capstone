...
export async function GET() {
  try {
    ...
    const averageGPA =
      validGPAs.length > 0
        ? validGPAs.reduce((acc, s) => acc + (s.predicted_gpa ?? 0), 0) /
          validGPAs.length
        : 0;
    ...
    const performanceCategories = Object.entries(catCounts).map(
      ([name, value]) => ({ name, value }),
    );
    ...
    const gradeDistribution = Object.entries(gradeGroups)
      .map(([grade, gpas]) => ({
        ...
        avgGPA:
          gpas.length > 0
            ? Math.round(
                (gpas.reduce((a, b) => a + b, 0) / gpas.length) * 100,
              ) / 100
            : 0,
        count: gpas.length,
      }))
    ...
    const ecaAnalytics = ecaBuckets.map(({ label, min, max }) => {
      const bucket = students.filter(
        (s) => s.eca_participation >= min && s.eca_participation < max,
      );
      const avgGPA =
        bucket.length > 0
          ? bucket
              .filter((s) => s.predicted_gpa !== null)
              .reduce((acc, s) => acc + (s.predicted_gpa ?? 0), 0) /
